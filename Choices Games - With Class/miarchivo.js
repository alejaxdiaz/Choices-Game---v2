...
class Character {
    ...
    addHealthPotion() {
        this.potions.heal++;
        alertUser(`You found a Health Potion!`);
    }
    ...
    useHealthPotion() {
        if (this.potions.heal > 0) {
            this.lives += 1;
            this.potions.heal -= 1;
            alertUser(`You used a health potion and restored a life! You have ${this.lives} lives left.`);
        } else {
    ...
    loseLife() {
        this.lives--;
        alertUser(`You have ${this.lives} lives left.`);
    }

    showInventory() {
        let inventoryMessage = `Inventory for ${this.name}:\n`;
        inventoryMessage += this.potions.heal > 0 ?
            `You have ${this.potions.heal} Health Potion(s).\n` :
    ...
    do {
        ...
        switch (secondChoice) {
            ...
            case "3":
                ...
                character.addHealthPotion(); // Add item to inventory if right choice is chosen 
                break;
            default:
                alertUser("Invalid choice, please try again.");
        }
    ...
    if (character.lives < 3) {
        ...
        if (showInventoryChoice.toLowerCase() === 'yes') {
            ...
            if (usePotionChoice.toLowerCase() === 'yes') {
                character.useHealthPotion(); // Corrected line
            }
        }
    }
